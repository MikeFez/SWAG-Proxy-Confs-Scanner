...
def get_file_contents(conf_file_path):
    with open(conf_file_path, "r") as f:
        contents = f.readlines()
    return contents


def parse_conf_configuration(file_contents):
    data = {"locations": {}}
    ...
    server_portion = re.sub(LOCATION_SEC_EXPR, "", file_contents)
    location_portions = {loc_name: loc_contents for loc_name, loc_contents in re.findall(LOCATION_SEC_EXPR, file_contents, re.MULTILINE)}
    ...
    for location_name, location_config in location_portions.items():
        ...
        data["locations"][location_name]['authelia'] = True if authelia_loc_enabled is not None else False

        upstream_app = re.search(UPSTREAM_APP_EXPR, location_config, re.MULTILINE)
        data["locations"][location_name]['upstream_app'] = upstream_app[1] if upstream_app is not None else None

        upstream_port = re.search(UPSTREAM_PORT_EXPR, location_config, re.MULTILINE)
        data["locations"][location_name]['upstream_port'] = upstream_port[1] if upstream_port is not None else None

        upstream_proto = re.search(UPSTREAM_PROTO_EXPR, location_config, re.MULTILINE)
        data["locations"][location_name]['upstream_proto'] = upstream_proto[1] if upstream_proto is not None else None
    return data
